        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6 relative">
          {steps.map((step, index) => (
            <div key={step.title} className="relative flex flex-col items-center text-center group">
              ...
              <motion.div
                initial={{ scale: 0.8 }}
                whileInView={{ scale: 1 }} viewport={{ once: true, margin: typeof window !== 'undefined' && window.innerWidth < 768 ? "100px" : "-100px" }}
                transition={{ duration: 0.5, delay: index * 0.1 }}
                className="w-20 h-20 rounded-2xl bg-white/10 md:backdrop-blur-md border border-white/20 flex items-center justify-center mb-5 relative z-10 shadow-lg group-hover:scale-110 group-hover:bg-white/20 transition-all"
              >
                {step.icon}
              </motion.div>
