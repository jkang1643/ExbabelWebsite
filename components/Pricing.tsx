        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {plans.map((plan, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <div className={`card bg-white/70 backdrop-blur-sm shadow-lg hover:shadow-xl transition-all duration-300 h-full rounded-3xl ${
                plan.highlighted ? 'border-2 border-primary scale-105' : 'border border-[#5a5d80]/20'
              }`}>
                {plan.highlighted && (
                  <div className="badge badge-primary absolute top-6 right-6 text-white rounded-full px-4">Popular</div>
                )}
                <div className="card-body p-8">
                  <h3 className="text-2xl font-bold text-neutral text-center mb-2">{plan.name}</h3>
                  <div className="text-center my-6">
                    <span className="text-5xl font-bold bg-gradient-to-br from-[#5a5d80] to-[#5a5d80] bg-clip-text text-transparent">${plan.price}</span>
                    <span className="text-neutral text-lg">/mo</span>
                    <p className="text-xs text-neutral mt-2">USD</p>
                  </div>
                  <ul className="space-y-3 mb-8">
                    {plan.features.map((feature, idx) => (
                      <li key={idx} className="flex items-start gap-3">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[#5a5d80] flex-shrink-0 mt-0.5" viewBox="0 0 20 20" fill="currentColor">
                              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                            </svg>
                            <span className="text-neutral text-sm leading-relaxed">{feature}</span>
                      </li>
                    ))}
                  </ul>
