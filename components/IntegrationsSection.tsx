...
const staggerContainer = {
  initial: { },
  whileInView: { },
  viewport: { once: true, margin: typeof window !== 'undefined' && window.innerWidth < 768 ? "100px" : "-100px" },
...
export default function IntegrationsSection() {
  return (
    <section className="py-24 px-6 relative z-10 bg-slate-50 overflow-hidden">
      ...
      <motion.div className="max-w-7xl mx-auto" initial="initial" whileInView="whileInView" viewport={{ once: true, margin: typeof window !== 'undefined' && window.innerWidth < 768 ? "100px" : "-100px" }} variants={staggerContainer}>
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {integrations.map((integration, idx) => (
            <motion.div 
              key={idx}
              variants={fadeInUp}
              className="bg-white/80 md:backdrop-blur-xl p-8 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] ring-1 ring-slate-900/5 hover:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] transition-all hover:-translate-y-1 group"
            >
